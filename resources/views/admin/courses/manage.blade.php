...
<body class="font-poppins text-[#0A090B]">
    <section id="content" class="flex">
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col gap-10 px-5 mt-5">
                @if(session('success'))
                <div id="success-alert" class="mx-5 mt-5 mb-2 p-4 rounded-lg bg-green-100 border border-green-300 text-green-800 font-semibold flex items-center gap-2">
            ...
            <div class="header ml-[70px] pr-[70px] w-[940px] flex items-center justify-between mt-10">
                ...
                <div class="relative">
                    ...
                    <div class="dropdown-menu absolute hidden right-0 top-[66px] w-[270px] flex flex-col gap-4 p-5 border border-[#EEEEEE] bg-white rounded-[18px] transition-all duration-300 shadow-[0_10px_16px_0_#0A090B0D]">
                        ...
                        <form method="POST" action="{{ route('dashboard.courses.destroy', $course) }}" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="flex gap-[10px] items-center text-[#FD445E] w-full" onclick="return confirmDelete('{{ $course->name }}')">
                                <div class="w-5 h-5">
                        ...
                        <div class="question-card w-full flex items-center justify-between p-4 border border-[#EEEEEE] rounded-[20px]">
                            ...
                            <div class="flex items-center gap-[14px]">
                                ...
                                <form method="POST" action="{{ route('dashboard.course_questions.destroy', $question) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button class="w-[52px] h-[52px] flex shrink-0 items-center justify-center rounded-full bg-[#FD445E]" onclick="return confirmDeleteQuestion('{{ $question->question }}')">
    ...
    <script>
        function confirmDelete(courseName) {
            return confirm(`Apakah Anda yakin ingin menghapus course "${courseName}"? Tindakan ini tidak dapat dibatalkan.`);
        }

        function confirmDeleteQuestion(questionText) {
            return confirm(`Apakah Anda yakin ingin menghapus question "${questionText}"? Tindakan ini tidak dapat dibatalkan.`);
        }
